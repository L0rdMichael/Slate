...
struct NewTaskView: View {
  ...
  @State private var dragEndLocation: CGPoint? = nil
      ...
      VStack {
        ...
        GeometryReader { geo in
          ZStack(alignment: .top) {
            ...
            .gesture(
              DragGesture(minimumDistance: 0)
                .onChanged { value in
                  ...
                  // Save drag end location in local coordinates
                  dragEndLocation = CGPoint(x: value.location.x, y: value.location.y)
                }
                .onEnded { value in
                  if isDragging {
            ...
            // Draw anchor line if dragging
            if isDragging, let dragEndLocation = dragEndLocation {
              DragAnchorLine(start: CGPoint(x: geo.size.width / 2, y: 24), end: dragEndLocation)
            }
          }
        }
        .frame(height: 40)
  ...
  // Custom shape for anchor line
  struct DragAnchorLine: View {
    let start: CGPoint
    let end: CGPoint
    var body: some View {
      ZStack {
...
struct TaskItemView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      Text(
        "\(formatTimeDigital(task.elapsed))"
          + (task.isTimed ? " / \(formatTimeDigital(task.duration))" : "")
      )
